...
def create_calculated_values(request):
	students = Student.objects.exclude(raw_passed=False).exclude(absent_code='4').exclude(absent_code='5')
	for s in students:
		calc_student, created = CalculatedStudent.objects.get_or_create(student=s)
		calc_student.apply_total_base()
